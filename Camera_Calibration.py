...
gray = []
for i in range(1,16) :
    tmp = cv2.imread(r'./Dataset_OpenCvDl_Hw2/Q2_Image/'+ str(i) + '.bmp' )
    img.append(tmp)
...
def show_result() :
    ...
    for i in range(1,16) :
        org = cv2.imread(r'./Dataset_OpenCvDl_Hw2/Q2_Image/'+ str(i) + '.bmp' )
        plt.subplot(1, 2, 1)
        plt.axis('off')
        plt.imshow(org)
        plt.title("Distorted img")
        h,  w = org.shape[:2]
        newcameramtx, roi = cv2.getOptimalNewCameraMatrix(intrisic_mtx, dist, (w,h), 1, (w,h))
        dst = cv2.undistort(org, intrisic_mtx, dist, None, newcameramtx)
        plt.subplot(1, 2, 2)
        plt.axis('off')
        plt.imshow(dst)
